...
export const ROLES_EXCEPT_USER = Object.values(ROLES).filter(
  (role) => role !== ROLES.ANONYMOUS && role !== ROLES.USER,
);

export const ROLES_ADMIN_AND_OPERATOR = Object.values(ROLES).filter(
  (role) => role === ROLES.OPERATOR || role === ROLES.ADMIN,
);
